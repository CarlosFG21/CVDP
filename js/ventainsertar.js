...
function agregarVenta() {
  ...
  function agregarVentaATabla(venta) {
    var tabla = document.getElementById('tablaVentas').getElementsByTagName('tbody')[0];
  
    // Verificar si el producto ya existe en la tabla
    var filaExistente = Array.from(tabla.rows).find(row => row.getAttribute('data-id') === venta.idproducto.toString());
  
    if (filaExistente) {
        alert("Ya existe un producto con este ID en la tabla de ventas.");
        return;
    }
    ...
    fila.setAttribute('data-id', venta.idproducto);
    ...
    cellProducto.textContent = venta.idproducto;
    cellCantidad.textContent = venta.cantidad;
    cellunidadm.textContent = venta.unidadm;
    cellPrecioUnitario.textContent = venta.precio;

    // Calcular y mostrar el total de la venta
    var totalVenta = venta.cantidad * venta.precio;
    cellTotal.textContent = 'Q' + totalVenta.toFixed(2);
    ...
    btnEliminarD.onclick = function() {
        ...
        var idProductoEliminar = venta.idproducto.toString();

        // Buscar la fila que corresponde al idproducto y eliminarla
        for (var i = 0; i < tabla.rows.length; i++) {
            if (tabla.rows[i].getAttribute('data-id') === idProductoEliminar) {
                tabla.deleteRow(i);
                break;
            }
        }
  ...
  function recalcularTotalGeneral() {
    ...
    for (var i = 0; i < tabla.rows.length; i++) {
      var totalVentaString = tabla.rows[i].cells[4].textContent.replace('Q', '');
      totalGeneral += parseFloat(totalVentaString);
    }
